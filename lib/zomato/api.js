...
export async function buildCookieHeader() {
  ...
  for (const url of ZOMATO_COOKIE_URLS) {
...
async function fetchOrderHistoryPage(resId, createdAt, postbackParams, headers, limit) {
  ...
  debugLog("[ZomatoOrders]", "get-all-v2 request", {
...
export async function fetchAllOrderHistorySnippets(options = {}) {
  ...
  while (hasMore && page < maxPages) {
    ...
    debugLog(
      "[ZomatoOrders]",
      `history page ${page + 1} — snippets: ${snippets.length}, hasMore: ${data?.hasMore}`
    );
  ...
  debugLog("[ZomatoOrders]", "total history snippets:", allSnippets.length);
  return allSnippets;
}
...
export async function fetchOrderDetails(tabId, authContext = null, signal = null) {
  ...
  debugLog("[ZomatoOrders]", "order-details request", { tabId });
  ...
  debugLog("[ZomatoOrders]", "order-details ok", { tabId, orderId: data.order.id });
  return data;
}
  ...
  const results = await mapWithConcurrency(
    ...
    async (tabId) => {
      try {
        ...
        debugWarn("[ZomatoOrders]", "order-details failed for", tabId, err.message);
        return null;
      }
    },
    {
